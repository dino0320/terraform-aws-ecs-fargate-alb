resource "aws_vpc" "this" {
  ...
  enable_dns_hostnames = true # Essential for VPC Endpoints and Private DNS
...
resource "aws_subnet" "public" {
  ...
  tags = merge(var.common_tags, {
    Name = "${var.app_name}-${var.env}-public-${count.index}"
  })
}
...
resource "aws_subnet" "private" {
  ...
  tags = merge(var.common_tags, {
    Name = "${var.app_name}-${var.env}-private-${count.index}"
  })
}
